...
local DEFAULT_CONFIG = {
   ...
   colors = require("draculanight.palette"),
   overrides = {},
}
...
---apply draculanight colorscheme
---@param configs DefaultConfig
local function apply(configs)
   local colors = configs.colors
   apply_term_colors(colors)
   local groups = require("draculanight.groups").setup(configs)
...
---setup draculanight colorscheme
---@param configs DefaultConfig?
...
---load draculanight colorscheme
local function load()
   if vim.version().minor < 7 then
      vim.notify_once("draculanight: you must use neovim 0.7 or higher")
      return
   end
